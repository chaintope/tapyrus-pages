...
<div class="updates-grid">
  {% for post in site.posts limit:6 %}
    ...
    {% if post.categories contains "core" %}
      {% assign category_class = "core" %}
      {% assign category_label = "Tapyrus Core" %}
    {% elsif post.categories contains "api" %}
      {% assign category_class = "api" %}
      {% assign category_label = "Tapyrus API" %}
    {% else %}
      {% assign category_label = "Release" %}
    {% endif %}
    <div class="update-card {{ category_class }}">
      <div class="update-date">{{ post.date | date: "%Y-%m-%d" }}</div>
      <a href="{{ post.url }}" class="update-title">{{ post.title }}</a>
      <p class="update-excerpt">{{ post.excerpt | strip_html | truncate: 80 }}</p>
      <span class="update-category">{{ category_label }}</span>
    </div>
  {% endfor %}
</div>
