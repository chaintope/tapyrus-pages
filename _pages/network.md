...
## テスト用ネットワーク
Chaintopeが運営するテスト用のネットワークは以下のようなパラメータを持ちます。
...
Chaintopeが運営するテスト用のネットワークは`1939510133`をネットワークIDとして持ちます。  
...
[Tapyrus Testnet Faucet](https://testnet-faucet.tapyrus.dev.chaintope.com/tapyrus/transactions){:target="_blank"}からTPCを受け取り、tapyrus-coreのCLIを用いてWalletを操作し、送金やトランザクション・ブロックの詳細を確認します。　　
